...
def extract_csv_from_zip(zip_list):
    ...
    for file in zip_list:
        try:
            ...
            print(f" Extraido: {csv_file}")
        except Exception as e:
            print(f" Exceção - Extraindo arquivo ZIP: {e}")
...
def process_csv(file):
    try:
        ...
        print(f" Processado: {file[0]}")
        return data_frame
    except Exception as e:
        print(f" Exceção - Processando CSV: {e}")
    return None
...
def merge_csv(csv_base):
    ...
    try:
        ...
        csv_base = csv_base.merge(companies_data, on="REGISTRO_OPERADORA", how="left")
...
def concat_csv(data_list):
    ...
    try:
        ...
        print(f" Exceção - Concatenando CSV: {e}")
    return data_frames
